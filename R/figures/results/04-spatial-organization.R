...
prepare_results_pca_data <- function(pca = prepare_pca_data()) {
  ...
  metadata <- read_csv(dataset_file("lake-metadata", "lake_metadata.csv"), show_col_types = FALSE)
  ...
  warming <- read_csv(
    dataset_file("warming-metrics", "lake_warming_metrics.csv"),
  ...
  tendency <- read_csv(
    dataset_file("trajectory-diagnostics", "trajectory_diagnostics.csv"),
    show_col_types = FALSE,
