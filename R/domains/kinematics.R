...
prepare_kinematics_data <- function() {
  ...
  raw_monthly <- read_csv(
    dataset_file("monthly-lswt", "monthly_temperature.csv"),
  ...
  metrics_path <- dataset_file("warming-metrics", "lake_warming_metrics.csv")
  ...
  trajectory_dir <- dataset_dir("trajectory-diagnostics")
