...
ggplot(ice_season_summary, aes(season, median, colour = cooling_period)) +
...
```{r}
#| label: fig-cooling-cohorts-map
#| fig-cap: "Spatial distribution of full-period and late-period cooling cohorts. Points are lakes, not area-weighted fields."
#| fig-width: 8
#| fig-height: 4.5

ggplot(cooling_map_data, aes(lon, lat, colour = cooling_period)) +
  the_geom_coastline() +
  geom_point(size = .22, alpha = .35) +
  coord_cartesian(xlim = c(-180, 180), ylim = c(-60, 85), expand = FALSE) +
  scale_colour_manual(values = c(
    "Cooling in both periods" = pal_direction[["cooling"]],
    "Full-period cooling only" = pal_direction_fill[["cooling"]],
    "Late-period cooling only" = pal_direction[["warming"]],
    "No cooling in either period" = col_text_soft
  ), name = NULL) +
...
@fig-cooling-cohorts-map is a cohort-location diagnostic, not a regional classification. It establishes where full-period and late-period cooling occur before any future catchment test.

> @fig-cooling-cohorts-map 只显示 cohort 位置，不把连续空间强行分区；它为未来流域连通性检验提供候选位置。
