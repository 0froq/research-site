---
title: "Restricted PCA Association Diagnostics"
aliases:
  - "/explorations/warming-acceleration/investigations/pca-association-diagnostics.html"
  - "/explorations/warming-acceleration/prose/pca-association-diagnostics.html"
---

> **Status:** Superseded by [External Interpretation of Spatially Balanced PCA](pca-external-interpretation.qmd), which adds the rotation-invariant PC2--PC3 joint response, parallel raw outcomes, and predeclared seasonal contrasts. Retained for audit of the earlier annual-only restricted panel.
...
```{r}
#| label: setup-pca-association-diagnostics
#| include: false
source("../../../R/core/style.R")
source("../../../R/figures/investigations/pca-association.R")
list2env(prepare_pca_association_sensitivity_data(), envir = environment())
```
...
```{r}
#| label: fig-pca-association-spatial-cv
#| fig-cap: "Spatial-block cross-validated R² for background versus restricted-ERA5 models. Three contiguous equal-area block sizes test whether the result depends on a single spatial hold-out scale. Negative values indicate worse-than-mean held-out prediction."
#| fig-width: 9
#| fig-height: 3.8

ggplot(cv_block_sensitivity, aes(block_label, spatial_block_cv_r2, colour = model, group = model)) +
  geom_hline(yintercept = 0, colour = col_axis, linewidth = wid_axis) +
  geom_line(linewidth = wid_inchart_line) +
  geom_point(size = 2) +
  facet_wrap(~component, nrow = 1) +
  scale_colour_manual(values = c("Background" = col_text_soft, "Background + ERA5" = pal_direction[["warming"]])) +
  labs(x = "Contiguous spatial block size", y = "Held-out R²", colour = NULL) +
