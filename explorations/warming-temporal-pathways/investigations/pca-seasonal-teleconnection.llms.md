...
For each lake, the JJA temperature series is linearly detrended. JJA temperature anomaly in year \\t\\ is then correlated with JJA NAO or AO in year \\t-1\\. Frozen seasonal states encoded as 0 °C are excluded. A lake correlation becomes Fisher-\\z\\, then an equal-area cell value is a pair-count-weighted mean across its lakes. We call this a **lagged seasonal association field**, not a sensitivity or forcing field: it is signed co-variation after only linear trend removal.

> 每湖 JJA 温度先线性去趋势，再将 \\t\\ 年 JJA 异常与 \\t-1\\ 年 JJA NAO/AO 相关。排除 0 °C 冰冻状态。逐湖相关转 Fisher-\\z\\，再按配对数加权汇总到等面积格网。称其为“滞后季节关联场”，不是敏感性场或 forcing 场：它只是去线性趋势后的有符号协变。
...
![](pca-seasonal-teleconnection_files/figure-html/fig-pca-jja-tele-models-1.png)

Figure 1: Spatial-block held-out R² for predicting each equal-area cell’s JJA lag-1 NAO/AO association field. All models include geography and lake morphology. PC2–PC3 is the only two-score model that consistently outperforms equal-dimensional PC1–PC2 and PC1–PC3 alternatives.
...
Adding available residence time, discharge, watershed area, volume, shoreline development, terrain slope, and reservoir fraction does not remove the PC2–PC3 association. The separate [lake-context exclusion check](../../../explorations/warming-temporal-pathways/investigations/pca-seasonal-teleconnection-lake-context.llms.md) gives the same-cell model comparison and all grid, LOCO, and LODO tests.

> 已有停留时间、流量、流域面积、体积、岸线发育度、坡度与水库比例，不能消除 PC2–PC3 关联。相同样本模型比较、格网、LOCO、LODO 见独立 lake-context exclusion check。

## Stability checks

![](pca-seasonal-teleconnection_files/figure-html/fig-pca-jja-tele-validation-1.png)

Figure 2: Held-out R² increment of PC2–PC3 over geography/morphology. Positive values persist across equal-area PCA grids, leave-one-continent-out refits, and leave-one-decade-out association calculations.
...
![](pca-seasonal-teleconnection_files/figure-html/fig-pca-jja-tele-fields-1.png)

Figure 3: Equal-area cell JJA lag-1 association fields. A value is the pair-count-weighted Fisher-z mean of lake-level correlations between detrended JJA LSWT in year t and JJA NAO/AO in year t-1. Colours show signed association, not causal forcing or significance.
...
![](pca-seasonal-teleconnection_files/figure-html/fig-pca-jja-nao-poles-1.png)

Figure 4: Raw annual and JJA LSWT trajectory composites for lower and higher quintiles of the JJA NAO lag-1 association field. Groups are display poles of a continuous field, not lake classes. Lines are equal-cell means; ribbons are cell interquartile ranges.
